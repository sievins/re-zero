      ...
      {items.map((item, index) => (
        <div key={item.id}>
          ...
          >
            ...
            <label
              htmlFor={item.id}
              className="ml-2 flex-grow text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
            >
              {item.label}
            </label>
            <button
              onClick={() => handleHighlight(item.id)}
              className="ml-28 text-sm text-primary hover:underline focus:outline-none sm:ml-96"
            >
              Mark
            </button>
          </div>
